...
受平台题库数据影响，**判断题给出的参考答案有时候是错的**，脚本会照平台返回的答案去选，所以判断题有可能被答错。碰到判断题建议自己看一眼，别完全依赖脚本。
...
- 浮层没出现：确认两个答题脚本都已启用，然后刷新页面。
- 浮层是空的或没自动答题：确认浮层上的「自动答题」按钮是高亮的；题目没加载完时等几秒，脚本会自动重试。
- 想临时关掉浮层：`Alt + Q` 显示 / 隐藏，标题栏可以拖动。
